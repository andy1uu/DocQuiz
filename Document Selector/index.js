...
let putCurrentSection = function(sectionNum){
    ...
    if(sectionNum >= paragraphs.length){
        ...
        $("#current-amount-selected").text("0.00%");
    ...
    //resets the current amount of the section that the user has selected
    $("#current-amount-selected").text((percentageOfCharactersSelected * 100).toFixed(2) + "%");
}
...
let selectSentence = function(element, sectionNum){
    ...
    if((percentageOfCharactersSelected < threshold) || element.hasClass("selected")){
        ...
        $("#no-more-selecting-warning").css("visibility", "visible");
        $("#no-more-selecting-warning").animate({opacity: 1}, 1500);
        $("#no-more-selecting-warning").animate({opacity: 0}, 1500); 
        setTimeout(function(){
            $("#no-more-selecting-warning").css("visibility", "hidden");
        }, 3000);
...
$(document).ready(function(){
    ...
    $("#threshold").text((threshold * 100).toFixed(2) + "%");
